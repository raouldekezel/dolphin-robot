...
| 2026-06-18 | [SPIKE-02](https://github.com/raouldekezel/dolphin-robot/issues/70) | Pre-D2 battery (E1–E6 + E3b PWS-offline): does the `clientToken` pattern surveyed in D1 survive every sequencing / atomicity / offline / ack edge before D2 picks the design?  | Yes, and a per-session UUID is sufficient (E5). AWS echoes our token on both `/update/accepted` and `/update/rejected`, accepts atomic combined `{mode, cycleTime}` writes (E1, RTT 89 ms), and treats every well-formed update as a new event (no no-op dedup, E6). `desired:null` is **PWS-firmware-authored** (E3b: 0 in a 102 s PWS-offline window, ~3 s post-reconnect) — corrects the D4 attribution. BUG-08 (#17) is reconfirmed in vivo on the E1 path: the reactive `Set cycle time` chain still overwrites our atomic `cycleTime=90` with the local default `150` unless D2 also gates the reactive branch on `accepted.clientToken ∈ in_flight`. | [2026-06-18_spike-02_pre-d2](2026-06-18_spike-02_pre-d2/findings.md)                                       |
