...
| 2026-06-20 | [FEAT-04](https://github.com/raouldekezel/dolphin-robot/issues/79)  | For a cycle started by the firmware's weekly scheduler (where the schedule object carries only `{mode}`, no `cycleTime`), which shadow field does the firmware use to decide the duration at trigger time?                                                         | `reported.cleaningModes[mode]` of the next-scheduled-run's `mode`. Confirmed at 11:49:04 CEST: weekly schedule for `mode=all` fired and the firmware reported `cycleInfo.cleaningMode = {mode:"all", cycleTime:180}`, matching `cleaningModes.all = 180` as last reported in the shadow before the trigger. `cleaningModes[mode]` is the only candidate inside the shadow that holds a per-mode duration. FEAT-04's new `sensor.{robot}_next_scheduled_cycle_time` must surface that field; if absent or non-int, return `None` (sensor goes `unavailable`).                                                                                                                                                                                                                                                                 | [2026-06-20_feat-04_cleaningmodes-source-confirmation](2026-06-20_feat-04_cleaningmodes-source-confirmation/findings.md) |
...
| 2026-06-21 | [BUG-13](https://github.com/raouldekezel/dolphin-robot/issues/47)   | After the PR #86 fix shipped in `v1.0.26b3-raoul.10`, do all three code branches behave as designed in vivo — silent E-B on docked pick, today-preserved live write on running pick, untouched Start path — and does a mode change on a running robot bump `rTurnOnCount` (the review's open question)? | Four scenarios all PASS. **Silent pick (docked, `all → stairs`)** — mode + cycleTime adopted, `pause()` lands ~84 ms after the cycleTime echo, robot stays `holdWeekly`, `rTurnOnCount` unchanged (58 → 58). **Start (`vacuum.start`)** — bare `set_cleaning_mode` path, no trailing `Set power state`, robot transitions `holdWeekly → on / init / scanning`, `rTurnOnCount` 58 → 59. **Two live mode-swaps (`stairs → floor → all` while cleaning)** — `is_active` branch correctly routes to bare `set_cleaning_mode`, no pause, firmware adopts each mode without `off` interlude, **`rTurnOnCount` stays 59 across both swaps** and `cycleStartTime` is not restamped — the firmware treats a running-robot mode-write as a continuation, not a restart. Settles the review's open question. | [2026-06-21_bug-13_in-vivo-validation](2026-06-21_bug-13_in-vivo-validation/findings.md)                                 |
