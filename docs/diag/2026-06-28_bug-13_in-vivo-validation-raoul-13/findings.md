...
- the cycle is not _restarted_ (`rTurnOnCount` unchanged, `vacuum.state` never leaves `cleaning`, no re-dock), but its **planned end-time moves** — see § _Operator-flagged behaviour to track separately_ below
