...
1. **`stairs` is a first-class user-selectable mode.** It is published by the integration on `desired.cleaningMode.mode`, accepted by the firmware, mirrored back on `reported.cleaningMode.mode`, and rendered as the operator-facing « Couverture complète » program in the Maytronics app. This matches the three independent pre-experiment indicators already analysed in [#31](../../../issues/31) (catalog entry with a duration, weekly schedule programmability, cycle starting *in* stairs, not *passing through* it).
...
3. **HA-initiated `cycleTime` writes win when the app is uninvolved.** The firmware adopted the integration's `180` within ~2 s of publish #48, and the operator's app showed an end time of T0 + 3 h 00. No counter-write race — this isolates the [BUG-08](../../../issues/17) race window to *app-initiated* mode changes only.
...
The PR's original design enforced the falsified *phase* premise. Required changes before merge (each with a regression test):
